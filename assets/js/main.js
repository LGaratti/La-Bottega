...
// 			});
	...
	// Style #2.
		$('.gallery')
			...
			.off('mouseenter', '.forward, .backward')  // remove mouseenter event
			.off('mouseleave', '.forward, .backward')  // remove mouseleave event
			.on('mousedown', '.forward, .backward', function(event) {
				if ($(window).width() > 768) {  // only apply changes for desktop screens
					var $this = $(this),
			...
			.on('mouseup mouseleave', '.forward, .backward', function(event) {
				// Clear move interval.
				clearInterval(this._gallery_moveIntervalId);
			});
			// ORIGINAL
